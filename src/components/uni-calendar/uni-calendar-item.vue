			...
			<view class="uni-calender__body-date-week">
				<block v-for="(day, index) in weeks" :key="index">
					<view :class="{
						...
						'uni-calender__date-current': ((day.date == canlender.date && !day.checked) || day.multipleBegin || day.multipleEnd) &&  canlender.month == day.month || !day.disable,
						...
						<view :class="{'uni-calender_check': day.checked, 'calender_check-begin': day.multipleBegin, 'calender_check-end': day.multipleEnd}" class="uni-calender_check-bg"></view>
					</view>
				</block>
			</view>
		</block>
...
<script lang="ts">
	...
	@Component({name:'UniCalendarItem'})
	export default class extends Vue{
		@Prop({default: ()=>{}}) canlender:object; 
		@Prop({default: false}) lunar: boolean;
